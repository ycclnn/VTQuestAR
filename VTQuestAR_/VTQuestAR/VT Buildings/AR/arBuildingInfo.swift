//
//  arBuildingInfo.swift
//  UIDemo
//
//  Created by zhennan on 2020/6/30.
//  Copyright © 2020 zhennan yao. All rights reserved.
//

import SwiftUI

struct arBuildingInfo: View {
    ...
    var body: some View {
        ...
        }.opacity(self.update == true ? 1 : 0).onReceive(showHideSender, perform: { (output: Bool) in
            // Whenever publisher sends new value, old one to be replaced
            self.update = output
        }).onReceive(buildingNameSender, perform: { (output: Building) in
            // Whenever publisher sends new value, old one to be replaced
