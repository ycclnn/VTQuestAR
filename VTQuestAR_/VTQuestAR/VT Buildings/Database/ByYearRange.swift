...
struct ByYearRange: View {
    ...
    var body: some View {
        ...
        Form {
            ...
            .navigationBarTitle(Text("Search by Year Range"), displayMode: .inline)
            // Move the Form up by keyboard height so that TextField shows above the keyboard
    ...
    func searchDatabase() {
        ...
        // The year building was built falls within the year range entered
        fetchRequest.predicate = NSPredicate(format: "yearBuilt >= %i AND yearBuilt <= %i", self.searchFieldValueLow, self.searchFieldValueHigh)
...
struct ByYearRange_Previews: PreviewProvider {
